...
These program was made for the data analysis of FRAP data obtained with the LSM 980 from Zeiss
_______________________________________________________________________________
...
This program is for the analysis of recovery-curves.
...
Define in this program :
     ...
     - If you take a bleach-control or background measurements into account (and from to where to fit the bleach function: x1, x2)
                  ____________________
        
FRAP measurement standards in order to use this program as it is:
    
    - R1 bleach acquisition, R2 bleachcontrol acquisition, R3 background, R4 comparison region
                  ____________________
...
# Specific folder that is being searched for csv files (needs to be customised) ----- if there aren't multiple data folders, then the last foldername can be used here and the path customized above needs to lead to this folder
Loop = ['']
#Loop = [0, 1, 2, ...]

# --------- specifiy file -------------------------------------------------------------------------
x = 1   # Number of the data folder in the loop starting with 0

f = 1  # Choose number of file in folder x (f_min = 1) 
